...
### TRAIN DATA IN BATCHES INSTEAD OF THE ENTIRE DATASET PER ADJUSTMENT TO THE FIT ###
...
class Loss:
    ...
    def calculate(self, output, y, *, include_regularization=False):
        ...
        # Add accumulated sum of losses and sample count to get mean loss of epoch across batches
    ...
    # Calculates accumlated loss of epoch across all batches
    def calculate_accumlated(self, *, include_regularization=False):
        # Calculate mean loss
    ...
    # Reset variables for accumlated loss
    def new_pass(self):
...
class Loss_BinaryCrossentropy(Loss):
    ...
    def forward(self, y_pred, y_true):
        ...
        # Calculate and return mean loss across all samples
        data_loss = np.mean(sample_losses)
        return data_loss
...
class Loss_MeanSquaredError(Loss):  # L2 loss
    ...
    def forward(self, y_pred, y_true):
        ...
        # Calculate loss
        data_loss = 2 * np.mean((y_true - y_pred)**2)
...
class Loss_MeanAbsoluteError(Loss):  # L1 loss
    ...
    def forward(self, y_pred, y_true):
        ...
        # Calculate loss
        data_loss = np.mean(np.abs(y_true - y_pred))
...
class Accuracy:
    ...
    # Calculate accuracy when using batches
    def calculate_accumlated(self):
        # Calcualte an accuracy
...
class Model:
    ...
    def train(self, X, y, *, epochs=1, batch_size=None, print_every=1, validation_data=None):
        ...
        if batch_size is not None:
            ...
            # Since int division rounds down, include last batch if not included already
        ...
        for epoch in range(1, epochs+1):
            ...
            for step in range(train_steps):
                ...
                # If batch size isn't set, train using 1 step and full dataset
                if batch_size is None:
                    batch_X = X
                    batch_y = y 
                else: # Otherwise, slice data into batch
            ...
            epoch_data_loss, epoch_regularization_loss = self.loss.calculate_accumlated(include_regularization=True)
            epoch_loss = epoch_data_loss + epoch_regularization_loss
            epoch_accuracy = self.accuracy.calculate_accumlated()
        ...
        if validation_data is not None:
            ...
            for step in range(validation_steps):
                ...
                validation_loss = self.loss.calculate_accumlated()
                validation_accuracy = self.accuracy.calculate_accumlated()
...
for epoch in range(EPOCHS):
    for step in range(steps):
        ...
        # Now perform forward pass, loss calc, backward pass and update parameters